...
function handleNextTurnClick() {
    if (!game.isTurnOver()) {
        showMessage('You must hold all 6 dice before ending your turn.');
        return;
    }
...
function updateUI() {
    ...
    const heldCount = game.diceSet.getHeldCount();
    const qualified = game.diceSet.isQualified();
    ...
    if (game.rollCount === 0) {
        ...
        rollBtn.textContent = `Roll Again (${heldCount} held)`;
    }
    ...
    if (game.isTurnOver()) {
        if (qualified) {
            nextTurnBtn.textContent = `Keep Score: ${turnScore} & End Turn`;
            showMessage(`Qualified! ${currentPlayer.name}'s score is ${turnScore}.`);
        } else {
            nextTurnBtn.textContent = 'End Turn (Score: 0)';
            showMessage(`${currentPlayer.name} did not qualify. Score: 0.`);
        }
    }
}
...
function renderDice() {
    ...
    const hasRolled = game.rollCount > 0;
    const diceEntities = ['?', '&#9856;', '&#9857;', '&#9858;', '&#9859;', '&#9860;', '&#9861;'];
    ...
    for (const die of game.diceSet.dice) {
        ...
        } else if (game.isTurnOver() && !game.diceSet.isQualified()) {
